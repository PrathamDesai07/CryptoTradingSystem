...
class Settings(BaseModel):
    ...
    max_open_positions_per_user: int = Field(ge=1)
    max_price_deviation_percent: float = Field(gt=0, le=100)
    max_concurrent_exchange_requests: int = Field(ge=1, le=100)
    max_consecutive_order_failures: int = Field(ge=1)
    order_circuit_breaker_cooldown_seconds: float = Field(gt=0)
    reconciliation_interval_seconds: float = Field(gt=0)
